package ae.network.nicardmanagementsdk.presentation.models

object Extra {
    const val EXTRA_NI_INPUT = "NIInput"
    const val EXTRA_NI_COPY_CLIPBOARD_TEXT = "CopyToClipboardMessage"
    const val EXTRA_NI_CARD_BACKGROUND = "CardBackgroundImage"
    const val EXTRA_NI_CARD_NAVIGATION_TITLE = "CardDetailsNavigationTitle"
    const val EXTRA_NI_CARD_ELEMENTS_CONFIG = "CardElementsConfig"

    const val EXTRA_NI_PIN_FORM_RESOURCES = "PinManagementResources"
    const val EXTRA_NI_PIN_FORM_TYPE = "NIPinFormType"
    const val EXTRA_NI_SUCCESS_RESPONSE = "NISuccessResponse"
    const val EXTRA_NI_ERROR_RESPONSE = "NIErrorResponse"
    const val EXTRA_NI_FRAGMENT_TOP_PADDING = "NIFragmentTopPadding"
    const val EXTRA_VIEW_PIN_START_TIME = "ViewPinStartTime"
    const val EXTRA_VIEW_PIN_STROKE_COLOR = "ViewPinStrokeColor"
}